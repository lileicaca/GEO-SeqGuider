...
# 如果本地有缓存，直接从缓存读取训练数据，否则从原始数据读取，并创建数据缓存
...
def train(args, train_dataset, model: PreTrainedModel) -> Tuple[int, float]:
    ...
    for num in train_iterator:
        ...
        # 整个测试集计算一个Acc均值来进行early stop的判断
        if 0 != args.model_task:
...
def main(args):
    ...
    logger.warning(
        "Process rank: -1, device: %s, n_gpu: %s, distributed training: False",
        args.device,
        args.n_gpu
    )
    ...
    if 0 == args.model_task:
        ...
        if args.model_name_or_path:
            llamaConfiguration = llama.LlamaConfig.from_pretrained(args.model_name_or_path, cache_dir=args.cache_dir)
        else:
            if args.model_config_file is None:
                raise Exception(f"During the pretrain phase, parameter --model_config_file must be set")
            with open(args.model_config_file) as file:
                config = json.load(file)
            llamaConfiguration = llama.LlamaConfig(vocab_size=config['vocab_size'],
        ...
        if args.model_name_or_path:
            ...
            model = llama.LlamaForCausalLM(llamaConfiguration)
    else:
        # initialize finetune model
        if args.model_name_or_path:
            # load model config
            llamaConfiguration = llama.LlamaConfig.from_pretrained(args.model_name_or_path, cache_dir=args.cache_dir)
            # The parameter num_labels = 1 indicates that the model will perform regression tasks
            llamaConfiguration.num_labels = 1
            # The parameter pad_token_id = 0 indicates that during the data processing stage,
            # data with inconsistent lengths will be filled with 0
            ...
            logger.error(f"During the fine-tuning phase, parameter --model_name_or_path must be set")

    # Transfer the model to GPU or CPU
    model.to(args.device)
    ##########################################################################
    # record model structure and model parameters
    logger.info("Model structure:")
    logger.info(model)
    num_params = sum(torch.tensor(p.shape).prod() for p in model.parameters())
    logger.info("Model Parameters:")
    logger.info(num_params)
    ###########################################################################
    # record all args parameters
    logger.info("Training/evaluation parameters %s", args)

    # Training
    if args.do_train:
        train_dataset = load_dataset_from_json(args, evaluate=False)
    ...
    if args.do_train:
        ...
        model.to(args.device)  # 加载到GPU/CPU进行训练
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train_label_file", default=None, type=str, help="The input training label file (a text file)."
    )
    parser.add_argument(
        "--model_config_file", default=None, type=str, help="The input training data file (a text file)."
    )
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="An optional input evaluation label file to evaluate the perplexity on (a text file).",
    )
    parser.add_argument(
        "--line_by_line",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="The model config for model initialization.",
    )
    parser.add_argument(
        "--config_name",
        default=None,
    ...
    parser.add_argument("--model_task", type=int, default=1,
                        help="Task for current model: 0 indicate pretrain, 1 indicate finetune")
    parser.add_argument("--ablation_ratio", type=float, default=1.0,
                        help="The percentage of training data, default to 1, means that all training sets participate in model fine-tuning. "
                             "If other input is used, the data will be loaded according to the input percentage for model fine-tuning.")
