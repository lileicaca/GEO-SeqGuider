...
def mainLB(dataFile: Path, outFolder: Path):
...
def mainForPrediction(dataFile: Path, outFolder: Path):
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--dataFile", default='data/crispData.xlsx', type=str, required=True,
        help="The relative or absolute path of the crisp data file that needs to be tokenized"
    )
    parser.add_argument(
        "--output_dir", default='input', type=str, help="Folder for storing output files."
    )
    parser.add_argument(
        "--prediction", default=0, type=int,
        help="A parameter value of 0 means that the tokenize labeled dataset is prepared for training the model, "
             "while a parameter value of 1 means the tokenize dataset to be predicted."
    )
    args = parser.parse_args()

    if 0 == args.prediction:
        mainLB(args.dataFile, args.output_dir)
    elif 1 == args.prediction:
        mainForPrediction(args.dataFile, args.output_dir)
    else:
        raise Exception("The prediction parameter is incorrect. Please check and try again.")
